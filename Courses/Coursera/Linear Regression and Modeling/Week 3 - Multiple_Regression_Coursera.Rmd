...
### ANSWER : Observational Study - Observational studies are ones where researchers observe the effect of a risk factor, diagnostic test, treatment or other intervention without trying to change who is or isn’t exposed to it. Cohort studies and case control studies are two types of observational studies. 
...
### ANSWER: False #4 The left skewness of the data suggests that the students are less likely to rate the professors highly. 
```{r score-dist}
...
#plot(x, y, main = "Main title",
     #xlab = "Male Upper", ylab = "Female Upper",
     #pch = 19, frame = FALSE)
# Add regression line
plot(x, y, col = c("red","green"), main = "Main title",
    xlab = "Male Upper", ylab = "Female Upper",
     pch = 19, frame = FALSE)
abline(lm(y ~ x, data = evals), col = "blue")


cor(x, y)
#correlation Coefficient = R= 0.6906175
...
### ANSWER:  score = 3.888 + (.0666 * bty_avg).  Slope: .06664
```{r summary-model}
# type your code for the Exercise here, and Knit
...
### ANSWER: TRUE The p-value: 5.083e-05
...
ggplot(data = m_bty, aes(x = .resid)) +
  ...
  xlab("Residuals")
```
##A# ANSWER   3. Nearly normal residuals: Residuals are right skewed, but the sample size is large, so this may not be an important violation of conditions. The results are left-skewed.
...
### ANSWER:  'bty_avg' is still a significant predictor with a p-value of 6.48e-06. 
...
###ANSER: 4.Teaching, Tenured, Tenure Track 
...
### Answer 1.0.12 points lower than minority professors, all else held constant. 
...
### ANSWER: Yes, the coeefficients and significants changed.  If there is no change, than the dropped variable was perfectly colinear to another variable.
```{r p-val-select}
# type your code for the Exercise here, and Knit
m_full_min1 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
