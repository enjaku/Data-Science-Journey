...
# Using hits yields an R-squared of .6419, compared to the R-squared with at_bats of .3729
...
````{r runs with on-base plus slugging (`new_obs`)}
...
````{r runs with on-base plus slugging (`new_slug`)}
...
````{r runs with on-base plus slugging (`new_onbase`)}
