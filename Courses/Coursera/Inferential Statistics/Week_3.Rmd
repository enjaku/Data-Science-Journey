...
**Answer: #3 - There are 27 NA's in the 1,000 observations**
...
**Answer: $H_0: \mu_{smoking} = \mu_{non-smoking}$; $H_A: \mu_{smoking} \ne \mu_{non-smoking}$**
...
**Answer:  The p-value is .0199, so we would reject the $H_0$ in favor of $H_A$**. *"If the "p" is low, the Null mist go"*.
...
# By default we get a confidence interval of 95%, but I added it in for GP.
...
```
**ANSWER: Comment - with a 90% confidence interval the interval is narrower, which makes sense as we're saying that we are less confident that the true mean falls within this even narrower band.** 
...
$H_0: \mu_{younger.mom} = \mu_{mature.mom}$; $H_A: \mu_{younger.mom} \ne \mu_{mature.mom}$


```{r gained-mature-ht}
# type your code for the Exercise here, and Knit

# Group by "mature" to look at the sample sizes
...
by_age <- nc %>% group_by(mature)
by_age %>% arrange(desc(mage))

## This was a bit hard.  I need to think through these a little more before I start coding.
## Take data (nc) and group it by the variable "mature and pass it into a new table "by_age

#"%>% arrange by desc "mage".  Scroll down the table and find the cut-off.

# Also, know what the variables are.  "fage is NOT female age, its fathers age.

```
**Answer:  The cut-off is 34-35 years old, which seems steep.**
...
**EXERCISE: Are younger mothers (<35yrs old) likely to have more smaller babies?
$H_0: \mu_{younger.mothers} = \mu_{older.mothers}$;
$H_A: \mu_{younger.mothers} \ne \mu_{older.mothers}$
...
# Group by mature, i.e., younger and older mothers.
...
#  Lets look at the data in a box plot

boxplot(weight~mature, data = nc)
  #  Not a lot there.  The IQR of mature moms is a bit bigger.

```

```{r  run the hypothesis test}
inference(y = weight, x = mature, data = nc, statistic = "mean", type = "ht", null = 0, 
          alternative = "twosided", method = "theoretical")
...
**Exercise:  At a 95% confidence level there is no difference between a baby's birth weight born to a younger or mature mother.  We cannot reject teh null hypthesis. 
