...
*No.  2 is false.  1499 is the mean, not the median*.
...
**A sample of 1000 will provide the most accurate information**
...
no_elements <- nrow(sample_means50)
  no_elements
# It's 15,000 because I ran it again with 30k reps to see how the hist changed
...
**Answer**: There are 25 ovservations
...
1. What does each observation in the sampling distribution represent? 
    *Each observation is the mean of that particular sample of 10, 50 or 100.*
    
2. How does the mean, standard error, and shape of the sampling distribution change as the sample size increases? 
    *The mean gets closer to the population mean, and the SE gets smaller.*
    
3. How (if at all) do these values change if you increase the number of simulations?
    *The SE decreases and the mean gets close to the pop. mean.*
...
**Answer** The variability of the sampling distribution decreases.
...
ames %>%
  sample_n(size = 50) %>%
  summarise(price_x_bar = mean(price))


```
**Answer:** The best point estimate for the population mean is $191,136.  It obviously changes everytime to knit the markdown.
...
# increase the binwidth to 1000
ggplot(data = sample_means50, aes(x = price_x_bar)) +
  geom_histogram(binwidth = 1000)

mean(sample_means50$price_x_bar)


```

**Answer:** The samplying distributuin is unimodal and relatively normal with a mean of $180,913, which changes slightly everytime the markdown is knit.
...
# increase the binwidth to 500
ggplot(data = sample_means150, aes(x = price_x_bar)) +
  geom_histogram(binwidth = 500)

mean(sample_means150$price_x_bar)
